from fastapi import FastAPI,Depends,HTTPException,UploadFile,File,Form,staticfiles
...
@app.post('/recipes')
async def add_recipe(title:str=Form(...),description:str=Form(...),image:UploadFile=File(...),db: Session = Depends(get_db),user: dict = Depends(get_current_user)):
    if user:
        image_url = await save_image(image)
        print(image_url)
        db_recipe = Recipe(title=title,description=description,owner_id=user["userid"],image_url = image_url) # type: ignore
        db.add(db_recipe)
        db.commit()
...
@app.put('/recipes/{id}')
async def update_recipe(id:int,title:str=Form(...),description:str=Form(...),image:UploadFile=File(),db:Session=Depends(get_db),user:dict=Depends(get_current_user)):
    if user:
        db_recipe = db.exec(select(Recipe).where(Recipe.id == id)).first()
        if db_recipe:
            if db_recipe.owner_id == user['userid']:
                ...
                if image:
                    db_recipe.image_url = await save_image(image)
                db.commit()
